...
%% Results
fileName = 'ietiTest1Results_iter.csv';
...
%     fprintf('\tConstruction of IETI-multipatch structure:\n')
%     tic();
%     mp_strct = cnstrct_mp_strct(mp_strct,nrbarr,false,1e-7);
%     t = toc();
%     fprintf('\tFinished construction of IETI-multipatch structure: %d s\n\n',t);
    ...
    for j=1:numel(degrees)
        for k = 1:numel(subh)
            ...
            % Measure local rank deficit
%             [deficit] = computeLocDeficit({mp_strct.patch_arr.G},{mp_strct.patch_arr.tree});
