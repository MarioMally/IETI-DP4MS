...
%% Results
fileName = 'ietiTest2Results_iter.csv';
...
err_curl2 = zeros(numel(subH),numel(degrees));
err_hcurl2 = zeros(numel(subH),numel(degrees));
err_l22 = zeros(numel(subH),numel(degrees));
...
for i=1:numel(subH)
    ...
    for j=1:numel(degrees)
        ...
        fprintf(fileID,'%i,%i,%i,',degrees(j),subH(i),subh(i));
        fprintf('\tStarted Solving:\n')
        tic;
        [sol2,multi2] = curl_solver_skeleton(mp_strct,true,fileID,1e-6,true);
        fprintf('\tFinished solving after %d s\n\n',toc);
        [err_hcurl2(i,j),err_l22(i,j),err_curl2(i,j)] = hcurl_error_computation(mp_strct,sol2);
        fprintf('\t\tL2-Error: %.2d\n\n',err_curl2(i,j));
        fprintf(fileID,'%.2d,',err_curl2(i,j));
