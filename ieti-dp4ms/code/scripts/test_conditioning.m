...
%% Setup
subh = round(2.^(1:0.5:5));
subH = [2];
degrees = 1;
 
%% Results
fileName = 'ietiCurlConditioning.csv';
folderName = 'computedData/';
upperDirectory = '../../';
fileID = fopen(strcat(upperDirectory,folderName,fileName),'w');
fprintf(fileID,['deg,sH,sh,arr,p,lam,' ...
    'pcg eigest.,iter,a\n']);
err_curl = zeros(numel(subH),numel(degrees),numel(subh));
err_hcurl = zeros(numel(subH),numel(degrees),numel(subh));
err_l2 = zeros(numel(subH),numel(degrees),numel(subh));
...
for i=1:numel(subH)
    ...
    for j=1:numel(degrees)
        for k = 1:numel(subh)
            ...
            curl_conditioning(mp_strct,fileID,1e-6);
